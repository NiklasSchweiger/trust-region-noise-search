...
def create_full_prot(
    ...
    n = atom37.shape[0]
    # PDB format uses 1-based residue indices, but we store 0-based internally
    # The to_pdb function will handle the conversion when writing
    residue_index = np.arange(n)  # 0-based internally, converted to 1-based in to_pdb
    chain_index = np.zeros(n)  # All residues in chain 0 (maps to 'A')
