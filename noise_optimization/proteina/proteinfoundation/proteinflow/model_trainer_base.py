...
class ModelTrainerBase(L.LightningModule):
    ...
    def configure_inference(self, inf_cfg, nn_ag):
        ...
        # Sanity checks - print configuration for debugging
        print("🔍 [Proteina Model] configure_inference called with:")
        print(f"  dt: {getattr(inf_cfg, 'dt', 'NOT SET')}")
        print(f"  self_cond: {getattr(inf_cfg, 'self_cond', 'NOT SET')}")
        print(f"  guidance_weight: {getattr(inf_cfg, 'guidance_weight', 'NOT SET')}")
        print(f"  autoguidance_ratio: {getattr(inf_cfg, 'autoguidance_ratio', 'NOT SET')}")

        if hasattr(inf_cfg, 'sampling_caflow'):
            sampling = inf_cfg.sampling_caflow
            print(f"  sampling_mode: {getattr(sampling, 'sampling_mode', 'NOT SET')}")
            print(f"  sc_scale_noise: {getattr(sampling, 'sc_scale_noise', 'NOT SET')}")
            print(f"  sc_scale_score: {getattr(sampling, 'sc_scale_score', 'NOT SET')}")
            print(f"  gt_mode: {getattr(sampling, 'gt_mode', 'NOT SET')}")
            print(f"  gt_p: {getattr(sampling, 'gt_p', 'NOT SET')}")

        if hasattr(inf_cfg, 'schedule'):
            schedule = inf_cfg.schedule
            print(f"  schedule_mode: {getattr(schedule, 'schedule_mode', 'NOT SET')}")
            print(f"  schedule_p: {getattr(schedule, 'schedule_p', 'NOT SET')}")

        print(f"  nn_ag: {'SET' if nn_ag is not None else 'None'}")
        print("🔍 [Proteina Model] configure_inference completed")
    ...
    def predict_step(self, batch, batch_idx):
        ...
        """
        # Sanity checks - print what predict_step is using
        print("🎯 [Proteina Model] predict_step called with:")
        print(f"  batch: nres={batch.get('nres', 'NOT SET')}, nsamples={batch.get('nsamples', 'NOT SET')}, dt={batch.get('dt', 'NOT SET')}")

        sampling_args = self.inf_cfg.sampling_caflow
        print(f"  sampling_args from inf_cfg: sampling_mode={sampling_args.get('sampling_mode', 'NOT SET')}")
        ...
        print(f"  autoguidance_ratio: {autoguidance_ratio}")
        print(f"  cath_code: {cath_code}")
        print(f"  fold_cond: {self.inf_cfg.get('fold_cond', False)}")
    ...
    def generate(
        ...
        # sys.stdout.flush()
        # print("=" * 80, flush=True)
        # print("🚀 [Proteina Model] generate() CALLED!", flush=True)
        # print(f"  nsamples: {nsamples}, n: {n}, dt: {dt}", flush=True)
        # print(f"  self_cond: {self_cond}, cath_code: {cath_code}", flush=True)
        # print(f"  guidance_weight: {guidance_weight}, autoguidance_ratio: {autoguidance_ratio}", flush=True)
        ...
        # print(f"  dtype: {dtype}, mask: {'SET' if mask is not None else 'None'}", flush=True)
        
        # Convert string defaults to float if needed (bug fix)
        ...
        # print(f"  → After conversion: sc_scale_noise={sc_scale_noise} (type: {type(sc_scale_noise)}), sc_scale_score={sc_scale_score} (type: {type(sc_scale_score)})", flush=True)
        # print("=" * 80, flush=True)
